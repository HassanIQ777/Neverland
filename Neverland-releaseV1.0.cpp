...
#define sl this_thread::sleep_for(chrono::milliseconds(750))
#ifdef _WIN32
...
int level = 0, XP = 0, next_level = 0, healAmount = 0, max_health = 100, heal = 0;
...
string enemy_list1[] = {"Goblin", "Centipede", "Tryonix", "Elf", "Aspid Minion", "Troll"};
string enemy_list2[] = {"Witch", "Wizard", "Aspid Minion", "Primal Aspid", "Chinchilasaur"};
string enemy_list3[] = {"Aspid Minion", "Aspid Warrior", "Primal Aspid", "Acid Hermit", "Acid Slime"}; // citizens
string enemy_list4[] = {"Aspid Guard", "Queen Servant", "Acidmold", "Aspid Minion"};
...
bool has_the_player_encountered_and_talked_to_the_first_villager_yet_or_not = false;
bool has_the_player_encountered_and_talked_to_the_second_villager_yet_or_not = false;
bool has_the_player_encountered_and_talked_to_the_third_villager_yet_or_not = false;
bool has_tamed_the_horse = false;
bool has_met_acid_watcher = false;
bool free_potion = true;
bool has_city_access = false;
bool has_the_player_encountered_and_talked_to_the_fourth_villager_yet_or_not = false;
...
void start()
{
	...
	sl;
	cls;
	centeredText("********************The Prophecy********************");
	cout << endl
		 << endl;
	sleep(1500);
	centeredText("In a vast kingdom, there exists a nurturing Queen,");
	cout << endl;
	sleep(3500);
	centeredText("So powerful yet caring, so easy yet keen.");
	cout << endl;
	sleep(3500);
	centeredText("Ages will pass, then a warrior will arise,");
	cout << endl;
	sleep(3500);
	centeredText("That warrior will bring this kingdom to its demise.");
	cout << endl;
	sleep(6500);
#ifdef __linux__
	...
	sl;
	print("You exit the cave and see a massive land extending beyond the horizon\n", 70);
...
void UI()
{
	...
	{
		cout << "*************************************************" << endl;
		cout << left << setw(5) << "Name: " << left << setw(18) << name << "| Health: " << health << endl;
		cout << left << setw(5) << "Race: " << left << setw(18) << race << "| XP: " << XP << endl;
		cout << left << setw(4) << "Level: " << left << setw(17) << level << "| Next Level: " << next_level << " XP" << endl;
		cout << left << setw(24) << "Distance: ???" << left << setw(8) << "| Money: " << money << " Zyniolax" << endl;
		cout << "*************************************************" << endl;
	}
	else
	{
		...
		cout << left << setw(5) << "Race: " << left << setw(18) << race << "| XP: " << XP << endl;
		cout << left << setw(4) << "Level: " << left << setw(17) << level << "| Next Level: " << next_level << " XP" << endl;
		cout << left << setw(4) << "Distance: " << left << setw(3) << dist << left << setw(11) << "Bosels"
...
void CombatHUD()
{
	...
	if (has_the_player_encountered_and_talked_to_the_fourth_villager_yet_or_not)
	{
		...
		cout << left << setw(6) << "Health: " << left << setw(12) << health << "|  Enemy Health: " << enemy_hp << endl;
		cout << left << setw(5) << "Level: " << left << setw(13) << level << "|  Enemy Level: " << enemy_level << endl;
...
void Combat()
{
	CombatHUD();
	string choice;
	int player_dmg = (((rand() % 10) + 5) + (((level * 7) / 2) - 6) * sword_upgrade);

	if (dist < 20)
	{
		enemy_dmg = ((rand() % 10) + 3) + (((enemy_level * 6) / 2) - 5);
	}
	else if (dist >= 20 && dist < 65)
	{
		enemy_dmg = (((rand() % 10) + 3) + (((enemy_level * 6) / 2) - 5)) * 1.2;
	}
	else if (dist >= 65 && dist < 96)
	{
		enemy_dmg = (((rand() % 10) + 3) + (((enemy_level * 6) / 2) - 5)) * 1.4;
	}
	else if (dist >= 96)
	{
		enemy_dmg = (((rand() % 10) + 3) + (((enemy_level * 6) / 2) - 5)) * 1.6;
	}
	...
	if (health > 0 && enemy_hp > 0)
	{
		...
		if (adrenaline >= 15 && has_the_player_encountered_and_talked_to_the_fourth_villager_yet_or_not)
		{
			cout << " U)se spell \n";
		}
		cout << " --> ";
		...
		{ // Attacking
			print("Attacking...", 40);
			cout << "You dealt " << player_dmg << " damage to the " << current_enemy << endl;
			enemy_hp -= player_dmg;
			if (adrenaline <= 26)
			{
				int randi = (rand() % 3) + 2;
		...
		else if (choice == "u" && adrenaline >= 15)
		{
			...
			cout << ceil(player_dmg * 2.5) << " damage to the " << current_enemy << endl;
			adrenaline -= 15;
...
void Attacking()
{
	...
	{
		int money_drop = abs((((rand() % 7) + 5) + ((past_enemy_hp * 0.10))) * money_charm);
		money += money_drop;
		enemy_hp = 0;
		CombatHUD();
		cout << "The " << current_enemy << " died and dropped " << money_drop << " Zyniolaxes" << endl;
		XP += enemy_xp;
		sleep(2000);
		HUD();
	}
...
void Moving()
{
	...
	if (choice == "m")
	{
		...
		else
		{
			...
			if (dist == 72 && !has_the_player_encountered_and_talked_to_the_fourth_villager_yet_or_not)
			{
				Shaman();
			}

			if (dist == 20 && !has_the_player_encountered_and_talked_to_the_second_villager_yet_or_not)
			{
				meetOldMan();
				HUD();
			}
			...
			if (dist == 95 && !has_defeated_golem)
			{
				...
				if (!has_talked_with_golem)
				{
					...
					sleep(1500);
					print("[ACID GOLEM]: *Laughs* nehehehehe, little one....you'd have to get through me first.\n", 70);
					sleep(2000);
	...
	else if (choice == "b")
	{
		if (dist > 0)
		{
			...
			int temp = (rand() % 100) + 1;
			if (temp <= 10)
			{ // much lower chance of encounters
				CreateEnemy();
				print("You encountered a ", 30);
				cout << current_enemy;
				print(", prepare to fight!\n", 30);
...
void BrewerNPC()
{
	...
	if (!has_the_player_encountered_and_talked_to_the_first_villager_yet_or_not)
	{
		...
		sl;
		print("[Goblin Brewer]: But hey, I'm not selling a thing for free mehehehe *Laughs*\n", 90);
		print("[Goblin Brewer]: You pay me money, you get what you want\n", 100);
		sl;
		print("\n[Goblin Brewer]: Come back later, I'll get my stuff to display\n", 100);
		has_the_player_encountered_and_talked_to_the_first_villager_yet_or_not = true;
		dist++;
		sleep(5000);
		HUD();
	}
	else if (has_the_player_encountered_and_talked_to_the_first_villager_yet_or_not)
	{
...
void respawn()
{
	...
	{
		cout << endl
			 << name << " was put to death by a " << current_enemy << endl;
	}
	else if (cha == 1)
	{
		cout << endl
			 << name << " got killed by a " << current_enemy << endl;
	}
	else
	{
		cout << endl
			 << current_enemy << " ended " << name << "'s day" << endl;
	}
	sleep(2000);
	XP = 0;
	print("Respawning", 80);
...
void Swordsmith()
{
	...
	if (!has_the_player_encountered_and_talked_to_the_third_villager_yet_or_not)
	{
		...
		print("\n[Swordsmith]: Everybody does, but who wields a blade worth molding?", 100);
		cout << "\n[" << name << "]: ";
		print("Look at my sword big guy, I'll give you money for forging it!", 100);
		print("\n[Swordsmith]: Huh money? Gaha! Now were talking", 100);
		sleep(3000);
		has_the_player_encountered_and_talked_to_the_third_villager_yet_or_not = true;
	}
	if (has_the_player_encountered_and_talked_to_the_third_villager_yet_or_not)
		;
	{
		...
		else if (choice == "2")
		{
			if (!has_bought_first_charm)
			{
				if (money >= 120)
				{
					...
					print("[Goblin Brewer]: Now you may profit more after defeating foes.\n", 150);
					sl;
					money -= 120;
		...
		if (choice == "1")
		{
			if (money >= sword_upgrade_cost && can_upgrade_sword)
			{
				...
				if (sword_upgrade >= 1.45)
				{
					...
					print("\n[Swordsmith]: By laws of this Kingdom, you can upgrade your blade no further. \n", 100);
					sleep(2000);
					OpenMenu();
				}
				sleep(2000);
			...
			{
				print("\n[Swordsmith]: You were prohibited from upgrading that sword, by law of this Kingdom.\nNow get out!!\n", 30);
				sleep(3000);
				HUD();
			}
...
{
	UI();
	print("\n[Old Man]: Beware of the monsters beyond this area, adventurer!\nThey don't hit for nothing!!", 70);
	has_the_player_encountered_and_talked_to_the_second_villager_yet_or_not = true;
	dist++;
	sleep(3500);
	cls;
}
...
void meetAcidWatcher()
{
	...
	print("\n[Acid Watcher]: And get ready for a battle as you've never seen.", 70);
...
void Shaman()
{
	...
	sl;
	print("\n[Acid Shaman]: Maybe you could try it? But I'm not sure of the side effects...", 60);
	sleep(2000);
	cout << "\n[" << name << "]: ";
	print("I will risk it, give me what you have and we'll see what happens.", 60);
	sl;
	print("\n[Acid Shaman]: Here goes nothing...", 50);
	sleep(3000);
	UI();
	print("\nYou unlocked a new ability: *Energy Burst*", 70);
	print("\nYou can use this spell on your enemies whenever your Adrenaline is 15 or higher.", 60);
	has_the_player_encountered_and_talked_to_the_fourth_villager_yet_or_not = true;
	sleep(5000);
	UI();
...
void CityGuardHUD()
{
	...
	cout << left << setw(18) << name << "  VS " << current_enemy << endl;
	cout << left << setw(6) << "Health: " << left << setw(12) << health << "|  Boss Health: " << enemy_hp << endl;
	cout << "********************************************" << endl;
}
	...
	if (health > 0 && enemy_hp > 0)
	{
		...
		{ // Attacking
			...
			if (health < 1)
			{
				...
				{
					cout << endl
						 << name << " was brutally massacred by the City Guardian." << endl;
				}
				else
				{
					cout << endl
						 << "The City Guardian wiped " << name << " against the floor." << endl;
				}
				sleep(3000);
				XP = 0;
				print("Respawning", 80);
...
{
	UI();
	print("\n\n[Acid Citizen 1]: Have you heard of that warrior that beat the City Guardian?", 60);
	sl;
	print("\n\n[Acid Citizen 2]: Yes what a battle it was, but do you think they'd make it into the castle?", 60);
	sl;
	print("\n\n[Acid Citizen 1]: To make it in they must beat the Acid Golem.", 60);
	sl;
...
void Golem()
{
	...
	if (health > 0 && enemy_hp > 0)
	{
		...
		{ // Attacking
			...
			if (health < 1)
			{
				...
				{
					cout << endl
						 << name << " was smashed to death by the Golem." << endl;
				}
				else if (chan == 1)
				{
					cout << endl
	...
	if (enemy_hp <= 0)
	{
		...
		centeredText("YOU WON!");
		sleep(2000);
		print("\n\n[ACID GOLEM]: powerful warrior, please don't harm the Queen", 80);
		sleep(1500);
		print("\n*Dies*", 100);
...
void Queen()
{
	if (!has_encountered_queen)
	{
		...
		{
			dist = 136;
			queen_hp = 5200; //(max_health * 10) + ((rand() % 180) + 20); // old formula
			QueenFight();
		}
	}
	QueenFight();
...
void QueenUI()
{
	...
	cout << "**************** BOSS FIGHT ****************" << endl;
	cout << left << setw(18) << name << "  VS " << current_enemy << endl;
	cout << left << setw(6) << "Health: " << left << setw(12) << health << "|  Queen Health: " << queen_hp << endl;
	cout << "Adrenaline: " << setw(8) << adrenaline << "|" << endl;
	cout << "********************************************" << endl;
}
	...
	if (health > 0 && queen_hp > 0)
	{
		...
		{ // Attacking
			...
			if (health < 1)
			{
				...
				{
					cout << endl
						 << name << " was destroyed by the Queen." << endl;
				}
				else
				{
					cout << endl
...
void Credits()
{
	...
	cout << endl
		 << endl;
	sleep(2000);
	centeredText("Directed By");
	centeredText("@HassanIQ777");
	cout << endl<< endl;
	sleep(2000);
	centeredText("Coding By");
	centeredText("@HassanIQ777");
	cout << endl<< endl;
	sleep(2000);
	centeredText("Game Design");
	centeredText("@HassanIQ777");
	cout << endl<< endl;
	sleep(2000);
	centeredText("Character Design");
	centeredText("@HassanIQ777");
	cout << endl<< endl;
	sleep(2000);
	centeredText("Game Idea");
	centeredText("@HassanIQ777");
	cout << endl<< endl;
	sleep(2000);
	centeredText("Special Thanks");
	centeredText("@Permiss1on_Denied");
	cout << endl<< endl;
	sleep(3000);
	print("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n", 100);
	sleep(2000);
	print("You had ventured out, met people, defeated monsters, and become a king for this kingdom.\n", 80);
	sleep(3000);
